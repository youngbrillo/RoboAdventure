...
namespace glib::towerdefense
{
    ...
    int EnemyGetNextPosition(int16_t currentX, int16_t currentY, int16_t* nextX, int16_t* nextY)
    {
        ...
        Vector2 gradient = PathFindingGetGradient(Vector3 { (float)currentX, 0, (float)currentY });
    ...
    Vector2 EnemyGetPosition(Enemy* enemy, float deltaT, Vector2* velocity, int* waypointPassedCount)
    {
        ...
        for (float t = 0.0f; t < deltaT; t += maxSimStepTime)
        {
            ...
            Vector2 target = Vector2{ (float)nextX,(float)nextY };
            ...
            if (Vector2DistanceSqr(target, lookForwardPos) <= pointReachedDistance2)
            {
                ...
                target = Vector2{ (float)nextX, (float)nextY };
    ...
    void EnemyUpdate()
    {
        ...
        for (int i = 0; i < enemyCount; i++)
        {
            ...
            for (int j = 0; j < towerCount; j++)
            {
                ...
                float distanceSqr = Vector2DistanceSqr(enemy->simPosition, Vector2 { (float)tower->x, (float)tower->y });
    ...
    EnemyId EnemyGetId(Enemy* enemy)
    {
        ...
        uint16_t index = enemy - enemies;
        return EnemyId{ index, enemy->generation };
    }
    ...
    Enemy* EnemyTryAdd(uint8_t enemyType, int16_t currentX, int16_t currentY)
    {
        ...
        if (spawn)
        {
            ...
            spawn->simPosition = Vector2{ (float)currentX,(float)currentY };
