...
namespace glib::towerdefense
{
    ...
    void PathFindingMapUpdate()
    {
        ...
        for (int i = 0; i < towerCount; i++)
        {
            ...
            // one cell. For now.
            if (!PathFindingFromWorldToMapPosition(Vector3 { (float)tower->x, 0.0f, (float)tower->y }, & mapX, & mapY))
            {
                continue;
            }
    ...
    void PathFindingMapDraw()
    {
        ...
        for (int x = 0; x < pathfindingMap.width; x++)
        {
            for (int y = 0; y < pathfindingMap.height; y++)
            {
                ...
                unsigned char rcom = fminf(colorV, 1.0f) * 255;
                Color color = distance < 0 ? BLUE : Color { rcom, 0, 0, 255 };
                Vector3 position = Vector3Transform(Vector3 { (float)x, -0.25f, (float)y }, pathfindingMap.toWorldSpace);
